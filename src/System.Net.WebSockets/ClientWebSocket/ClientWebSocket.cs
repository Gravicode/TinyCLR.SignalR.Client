...
namespace System.Net.WebSockets
{
    ...
    public class ClientWebSocket : WebSocket, IDisposable
    {
        ...
        /// </summary>
        public SslVerification SslVerification { get; set; } = SslVerification.Required;
        ...
        public void Connect(string uri, ClientWebSocketHeaders headers = null)
        {
            ...
            try
            {
                ...
                if (IsSSL)
                {
                    ...
                    if (SslVerification != SslVerification.None && _certificate != null)
                    {
            ...
            if (bytesRead == bufferStart.Length)
            {
                if (Encoding.UTF8.GetString(bufferStart, 0, bufferStart.Length).ToLower() == beginHeader)
                {
                    ...
                    if (bytesRead > 20)
                    {
                        ...
                        var conStr = ((string)headers["connection"]).ToLower();
                        var upgradeStr = ((string)headers["upgrade"]).ToLower();
                        var acceptStr = (string)headers["sec-websocket-accept"];
                        if (conStr == "upgrade" && upgradeStr == "websocket" && acceptStr == swkaSha1Base64)
                        {
